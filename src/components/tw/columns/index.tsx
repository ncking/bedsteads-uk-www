...
export const TextBlock = ({ className, ...rest }) => (
  <div className={cx(styles.textBlock, className)} {...rest}></div>
)

export const H1 = ({ children, className }) =>
  children
    ? (
        <Column className={cx('col-span-full', styles.h1, className)}>
          <TextBlock>
            <h1>{children}</h1>
          </TextBlock>
        </Column>
...
export const MainBlock = ({
  ...
  return (
    <Columns
      className={cx('cols-1 md:cols-2', className, styles.mainBlock)}
    >
      <H1 className={classNames[0]}>{title}</H1>
      <TextBlock className={[classNames[1]]}>
        {children.length ? children[0] : children}
      </TextBlock>
      <TextBlock className={[classNames[2]]}>{children[1]}</TextBlock>
    </Columns>
  )
}
