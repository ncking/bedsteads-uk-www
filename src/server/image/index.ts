...
const stockMatcher = new RegExp(
    '/image/stock/(?<dec>[0-9]+)/(?<id>[0-9]+)/(?<file>[0-9]+),t_(?<token>(thumb-p|thumb|main-c|twitter|main|tile-p|tile-l|tile|gallery)+)(,s_)?(?<size>([0-9])+)?',
    'i',
)
const mainSizes = [
...
export const templates = async (args, httpError) => {
    ...
    try {
        ...
        switch (template) {
            ...
            case 'main':
                if (0 === size && !isFurniture) {
                    // we need to make furniture a differnt tag ... or run behind cookie
